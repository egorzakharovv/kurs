	...
	private fun testRest(
		...
		withTestApplication({
			...
			transaction {
				...
				productTable.insertAndGetIdItem(Product("INITIAL")).value
			...
			var asd = handleRequest(HttpMethod.Get, "$testPath/${users?.first()?.id}").run {
				assertStatus(HttpStatusCode.OK)
				val user = parseResponse(Post.serializer())
			...
			handleRequest(HttpMethod.Get, "$testPath/${items?.first()?.id}").run {
				assertStatus(HttpStatusCode.OK)
				val item = parseResponse(Post.serializer())
			...
			val threeNew = Post("one new", 1,three.id)
			println("\n--putin, ${three.id}\n")

			handleRequest(HttpMethod.Put, "$testPath/${threeNew.id}") {
				setBodyAndHeaders(Json.encodeToString(Post.serializer(), threeNew))
			}.run {
				assertStatus(HttpStatusCode.OK)
			}
			println("\n--input\n")
